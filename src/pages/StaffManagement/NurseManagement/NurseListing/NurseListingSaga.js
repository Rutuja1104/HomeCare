...
import {
    CERTIFICATIONS_INFO_MAPPING_KEYS,
    PREVIOUS_EMPLOYMENT_ADDRESS_KEY_MAPPING,
    PREVIOUS_EMPLOYMENT_INFO_MAPPING_KEYS,
    REFERENCES_INFO_MAPPING_KEYS
} from '../../../JobPostings/JobApplicationDetails/constants';
...
export const {
    ...
    getAllNurseList: (payload) => {
        return {
            type: 'NURSE_LISTING/GET_ALL_NURSES',
            payload
        };
    },
    putValidateUserByAgencyAdmin: (payload) => {
        return {
            type: 'NURSE_LISTING/DEACTIVATE_NURSE',
            payload
        };
    },
    getNurseById: (payload) => {
        return {
            type: 'NURSE_LISTING/GET_NURSE_DETAILS_BY_ID_2',
            payload
        };
    },
    putNurseById: (payload) => {
        return {
            type: 'NURSE_LISTING/UPDATE_NURSE',
            payload
        };
    },
...
function* getAllNurseListAsync(action) {
    try {
        yield put(setLoadingState({ state: PAGE_STATE.LOADING, message: 'Fetching Nurse List' }));
        const { token, agencyId, page, limit, status, firstName, lastName, role } = action.payload;
        const response = yield AdminDataService.getAllNurseList(
            token,
            agencyId,
            page,
            limit,
...
function* getNurseByIdAsync(action) {
    try {
        ...
        if (response.status === 200) {
            let certificationsInfo = General.generateDataFromArrayOfObject(
                CERTIFICATIONS_INFO_MAPPING_KEYS,
                response.data.certifications
            );
            let contactsInfo = General.generateDataFromArrayOfObject(
                REFERENCES_INFO_MAPPING_KEYS,
            ...
            let employmentsInfo = General.generateDataFromArrayOfObjectArray(
                PREVIOUS_EMPLOYMENT_INFO_MAPPING_KEYS,
                response.data.employments,
                { address: PREVIOUS_EMPLOYMENT_ADDRESS_KEY_MAPPING }
            );
...
function* putNurseByIdAsync(action) {
    try {
        yield put(setLoadingState({ state: PAGE_STATE.LOADING, message: 'Loading...' }));
...
function* rootSaga() {
    yield all([
        ...
        takeLatest(putNurseById().type, putNurseByIdAsync),
    ]);
}
