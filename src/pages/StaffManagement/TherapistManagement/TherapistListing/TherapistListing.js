...
import { componentKey, setAutoCompleteAddressFields, setDataToUpdate, setPaginationState, setSelectedTherapist, setSelectedTherapistList, setShowDeleteTherapistModal, setUpdateTherapiest } from './TherapistListingSlice'
...
import { getAllTherapistList, putTherapiestById, putValidateUserByAgencyAdmin } from './TherapistListingSaga'
import TextInput from '../../../../components/input/textinput/TextInput'
import ButtonDropdown from '../../../../components/buttonDropdown/ButtonDropdown'
import SelectDropdown from '../../../../components/select/Select'
import { THERAPIST_ROLES } from './constants'
...
const TherapistListing = () => {
    ...
    const { allTherapistList, loadingState, selectedTherapistList, PaginationState, showDeleteTherapistModal, selectedTherapist, dataToUpdate } = useSelector(state => state[componentKey])
    const [clickedRowIndex, setClickedRowIndex] = useState(-1);
    const [openUpdateTherapiestModal, setUpdateTherapiestModal] = useState(false)

    const ACTION_BUTTONS = [
        {
            text: "View Details",
            onClickCb: (row, idx) => {
                dispatch(setSelectedTherapist(row))
                navigate(`/${STAFF_MANAGEMENT_ROUTES.STAFF_MANAGEMENT}/${STAFF_MANAGEMENT_ROUTES.CHILD_ROUTS.NURSE_DETAILS}?nid=${row.id}&type=Therapist`)
            }
        },
        {
            text: "Update Details",
            onClickCb: (row, idx) => {
                setUpdateTherapiestModal(true)
                dispatch(setSelectedTherapist(row))
        ...
        ...(PaginationState.Status == "Active" ?
            [{
                ...
                onClickCb: (row, idx) => {
                    dispatch(setSelectedTherapist(row))
                    dispatch(setShowDeleteTherapistModal(true))
                }
            }] :
            [{
                text: "Activate",
    ...
    const DROPDOWN_BUTTON_ACTION = [
        {
            ...
            onClickCb: () => {
                ...
                dispatch(getAllTherapistList({ token, agencyId, status: "Active", page: PaginationState.PageNumber, limit: PaginationState.PageSize, firstName: PaginationState.FilterText, lastName: "", role: PaginationState.Role }))
            }
        },
        {
            title: "De-Activate",
            onClickCb: () => {
                ...
                dispatch(getAllTherapistList({ token, agencyId, status: "De-Activate", page: PaginationState.PageNumber, limit: PaginationState.PageSize, firstName: PaginationState.FilterText, lastName: "", role: PaginationState.Role }))
            }
        }
    ]
    ...
    useEffect(() => {
        dispatch(setHeaderLabel("Therapist Management"))
    }, [])

    useEffect(() => {
        const debounceTimer = setTimeout(() => {
            dispatch(getAllTherapistList({ token, agencyId, status: PaginationState.Status, page: PaginationState.PageNumber, limit: PaginationState.PageSize, firstName: PaginationState.FilterText, lastName: "", role: PaginationState.Role }))
        }, 300);
    ...
    const onUpdateChangeHandler = (event, rules) => {
        ...
        if (rules) {
            ...
            dispatch(setUpdateTherapiest({ [name]: { value, errors, rules } }));
        } else {
            dispatch(setUpdateTherapiest({ [name]: { value } }));
        }
    };
    ...
    return (
        <Loadable loadingStates={loadingState.state} loadingMessage={loadingState.message}>
                ...
                <div className='episode-filter'>
                    <SelectDropdown
                        ...
                        onChangeCb={(event) => {
                            ...
                            dispatch(getAllTherapistList({ token, agencyId, status: PaginationState.Status, page: PaginationState.PageNumber, limit: PaginationState.PageSize, firstName: PaginationState.FilterText, lastName: "", role: event.target.selectedOption.value }))
                        }}
                        defaultValue={PaginationState.NursesRole}
                        options={THERAPIST_ROLES}
                    />
                    <TextInput
                        type='text'
            ...
            <RichGrid
                data={allTherapistList}
                columns={NURSE_LISTING_COLUMNS}
                selectable={false}
                extractRowKey={(row) => row.id}
                onSelectionChangeCallBack={(selectedData) => dispatch(setSelectedTherapistList(selectedData))}
                onHeaderSelectionChangeCallBack={(selectedData) => dispatch(setSelectedTherapistList(selectedData))}
                selectedRows={selectedTherapistList}
                paginationProps={{
                    ...
                    onChangeTakeCb: (newTake) => {
                        ...
                        dispatch(getAllTherapistList({ token, agencyId, status: PaginationState.Status, page: 1, limit: newTake, firstName: PaginationState.FilterText, lastName: "", role: PaginationState.Role }))
                    },
                    onChange: (page) => {
                        dispatch(getAllTherapistList({ token, agencyId, status: PaginationState.Status, page: page, limit: PaginationState.PageSize, firstName: PaginationState.FilterText, lastName: "", role: PaginationState.Role }))
            ...
            <ConfirmationModal
                ...
                onSuccessCb={() => {
                    dispatch(putValidateUserByAgencyAdmin({ nurseId: selectedTherapist.id, data: { onboardingStatus: PaginationState.Status == "Active" ? "De-Activate" : "Active" }, token, agencyId, PaginationState }))
                }}
                toggle={() => dispatch(setShowDeleteTherapistModal(!showDeleteTherapistModal))}
                successButtonText={PaginationState.Status == "Active" ? "De-Activate" : "Active"}
            />
            {openUpdateTherapiestModal && (
                <BasicModal
                    open={openUpdateTherapiestModal}
                    title={"Update Therapiest Details"}
                    closeButtonHandler={() => setUpdateTherapiestModal(false)}
                    handleClose={() => setUpdateTherapiestModal(false)}
                    customStyle={{
                        width: '55%'
                    }}
                >
                    ...
                    <div className="nextbuttons">
                        <Button
                            ...
                            onClickCb={() => {
                                setUpdateTherapiestModal(false);
                            }}
                        >
                            Back
                        </Button>
                        <Button
                            ...
                            onClickCb={() => {
                                ...
                                if (!GeneralValidator.validateRequiredFields(dataToUpdate)) {
                                    const id = selectedTherapist?.id;
                                    dispatch(
                                        putTherapiestById({agencyId, id, dataToUpdate, token, selectedTherapist, PaginationState})
                                    );
                                    setUpdateTherapiestModal(false);
                                } else{
