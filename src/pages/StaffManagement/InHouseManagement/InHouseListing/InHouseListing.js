...
import { componentKey, setAutoCompleteAddressFields, setDataToUpdate, setPaginationState, setSelectedNurse, setSelectedNurseList, setShowDeleteNurseModal, setUpdateHouseStaff } from './InHouseListingSlice'
import { setHeaderLabel } from '../../../../layouts/LayoutSlice'
import { IN_HOUSE_STAFF_ROLES } from './constants'
import { getAllNurseList, putOfficeStaffById, putValidateUserByAgencyAdmin } from './InHouseListingSaga'
...
const InHouseListing = () => {
    ...
    const { allNurseList, loadingState, selectedNurseList, PaginationState, showDeleteNurseModal, selectedNurse, dataToUpdate } = useSelector(state => state[componentKey])
    const [openUpdateOfficeStaffModal, setUpdateOfficeStaffModal] = useState(false)
    ...
    const ACTION_BUTTONS = [
        ...
        },
        {
            text: "Update Details",
            onClickCb: (row, idx) => {
                setUpdateOfficeStaffModal(true)
                dispatch(setSelectedNurse(row))
    ...
    const DROPDOWN_BUTTON_ACTION = [
        {
            ...
            onClickCb: () => {
                ...
                dispatch(getAllNurseList({ token, agencyId, status: "Active", page: PaginationState.PageNumber, limit: PaginationState.PageSize, firstName: PaginationState.FilterText, lastName: "", role: PaginationState.Role }))
            }
        },
        {
            title: "De-Activate",
            onClickCb: () => {
                ...
                dispatch(getAllNurseList({ token, agencyId, status: "De-Activate", page: PaginationState.PageNumber, limit: PaginationState.PageSize, firstName: PaginationState.FilterText, lastName: "", role: PaginationState.Role }))
            }
        }
    ]
    ...
    useEffect(() => {
        dispatch(setHeaderLabel("Office Staff Management"))
    }, [])

    useEffect(() => {
        const debounceTimer = setTimeout(() => {
            dispatch(getAllNurseList({ token, agencyId, status: PaginationState.Status, page: PaginationState.PageNumber, limit: PaginationState.PageSize, firstName: PaginationState.FilterText, lastName: "", role: PaginationState.Role }))
        }, 300);
    ...
    const onUpdateChangeHandler = (event, rules) => {
        ...
        if (rules) {
            ...
            dispatch(setUpdateHouseStaff({ [name]: { value, errors, rules } }));
        } else {
            dispatch(setUpdateHouseStaff({ [name]: { value } }));
        }
    };
    ...
    return (
        <Loadable loadingStates={loadingState.state} loadingMessage={loadingState.message}>
                ...
                <div className='episode-filter'>
                    <SelectDropdown
                        ...
                        onChangeCb={(event) => {
                            ...
                            dispatch(getAllNurseList({ token, agencyId, status: PaginationState.Status, page: PaginationState.PageNumber, limit: PaginationState.PageSize, firstName: PaginationState.FilterText, lastName: "", role: event.target.selectedOption.value }))
                        }}
                        defaultValue={PaginationState.NursesRole}
                        options={IN_HOUSE_STAFF_ROLES}
                    />
                    <TextInput
                        type='text'
            ...
            <RichGrid
                ...
                paginationProps={{
                    ...
                    onChangeTakeCb: (newTake) => {
                        ...
                        dispatch(getAllNurseList({ token, agencyId, status: PaginationState.Status, page: 1, limit: newTake, firstName: PaginationState.FilterText, lastName: "", role: PaginationState.Role }))
                    },
                    onChange: (page) => {
                        dispatch(getAllNurseList({ token, agencyId, status: PaginationState.Status, page: page, limit: PaginationState.PageSize, firstName: PaginationState.FilterText, lastName: "", role: PaginationState.Role }))
            ...
            <ConfirmationModal
                ...
                onSuccessCb={() => {
                    dispatch(putValidateUserByAgencyAdmin({ nurseId: selectedNurse.id, data: { onboardingStatus: PaginationState.Status == "Active" ? "De-Activate" : "Active" }, token, agencyId, PaginationState }))
                }}
            ...
            />
            {openUpdateOfficeStaffModal && (
                <BasicModal
                    open={openUpdateOfficeStaffModal}
                    title={"Update Nurse Details"}
                    closeButtonHandler={() => setUpdateOfficeStaffModal(false)}
                    handleClose={() => setUpdateOfficeStaffModal(false)}
                    customStyle={{
                        width: '55%'
                    }}
                >
                        ...
                        <div className="information">
                            <div className="block">
                                <TextInput
                                    ...
                                    name="firstName"
                                    label="Firstname"
                                    onChangeCb={(event, rules) => onUpdateChangeHandler(event, dataToUpdate?.firstName.value)}
                    ...
                    <div className="nextbuttons">
                        <Button
                            ...
                            onClickCb={() => {
                                setUpdateOfficeStaffModal(false);
                            }}
                        >
                            Back
                        </Button>
                        <Button
                            ...
                            onClickCb={() => {
                                ...
                                if (!GeneralValidator.validateRequiredFields(dataToUpdate)) {
                                    ...
                                    dispatch(
                                        putOfficeStaffById({agencyId, id, dataToUpdate, token, selectedNurse, PaginationState})
                                    );
                                    setUpdateOfficeStaffModal(false);
                                } else{
