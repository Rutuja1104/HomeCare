...
import { componentKey, setSelectedProgressNotes, setProgressNotesPaginationState, setSelectedProgressNote, setShowDeleteProgressNoteModal } from '../../AdminEpisodeManagementSlice'
import { deleteProgressNote, getProgressNotesByEpisodeId } from '../../AdminEpisodeManagementSaga'
...
const ProgressNotes = () => {
    ...
    const ACTION_BUTTONS = [
        ...
        // },
        {
            text: "View",
            onClickCb: (row) => {
                navigate(`/${EPISODE_MANAGEMENT.EPISODE_MANAGEMENT}/${EPISODE_MANAGEMENT.CHILD_ROUTS.ADD_PROGRESS_NOTES}/${episodeId}?pi=${row.id}`)
            }
        },
    ];
    ...
    const DROPDOWN_BUTTON_ACTION = [
        {
            title: "Add New Note",
            onClickCb: () => navigate(`/${EPISODE_MANAGEMENT.EPISODE_MANAGEMENT}/${EPISODE_MANAGEMENT.CHILD_ROUTS.ADD_PROGRESS_NOTES}/${episodeId}`)
        }
    ]
    return (
        <React.Fragment>
            <div className='mb-2 d-flex justify-content-between'>
                <SelectDropdown
                    ...
                    onChangeCb={(event) => {
                        ...
                        dispatch(setProgressNotesPaginationState({ limit: 10 }))
                        dispatch(getProgressNotesByEpisodeId({ agencyId, episodeId, status: event.target.selectedOption.value, limit: 10, pageNumber: 1, token }))
                    }}
            ...
            <RichGrid
                ...
                paginationProps={{
                    ...
                    total: ProgressNotesPaginationState.totalNotes,
