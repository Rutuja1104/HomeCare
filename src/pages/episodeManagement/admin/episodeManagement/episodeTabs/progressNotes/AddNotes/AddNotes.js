...
import { setAddNoteStates, setExistingProgressNoteDetails, setIsNavigateToProgressNoteList, setSelectedProgressNoteAssignee } from '../../../AdminEpisodeManagementSlice'
import { componentKey as adminEpisodeManagementComponentKey, setAddNoteFieldTouch, setProgressNoteSignature } from "../../../AdminEpisodeManagementSlice"
...
import { getAssigneeByEpisodeId, getEmergencyVitals, getEpisodeDetailsById, getProgressNoteById, patchUpdateExistingProgressNotes, postCreateProgressNote } from '../../../AdminEpisodeManagementSaga'
...
const AddNotes = () => {
    ...
    useEffect(() => {
        if (progressDetailById.id) {
            ...
            copiedProgressDetails.DigitalSignature.value = progressDetailById.digitalSignature
    ...
    useEffect(() => {
        if (isNavigateToProgressNoteList) {
            navigate(`/${EPISODE_MANAGEMENT.EPISODE_MANAGEMENT}/${EPISODE_MANAGEMENT.CHILD_ROUTS.EPISODE_DETAILS}/${episodeId}?at=3`)
    ...
    return (
        <Loadable loadingStates={loadingState.state} loadingMessage={loadingState.message}>
            <Container>
                ...
                <Base64Image base64={addNoteStates.DigitalSignature.value} />
                <Button className='button-width mt-5 me-3' variant={BUTTON_TYPE.PRIMARY_LIGHT_WITH_BORDER} onClickCb={() => navigate(`/${EPISODE_MANAGEMENT.EPISODE_MANAGEMENT}/${EPISODE_MANAGEMENT.CHILD_ROUTS.EPISODE_DETAILS}/${episodeId}?at=3`)}>Back</Button>
                {/* <SignaturePad savedSignature={addNoteStates.DigitalSignature.value} saveSignatureCb={handleSaveSignatureClick} onPrevCb={() => navigate(`/${EPISODE_MANAGEMENT.EPISODE_MANAGEMENT}/${EPISODE_MANAGEMENT.CHILD_ROUTS.EPISODE_DETAILS}/${episodeId}?at=3`)} isPreviousAllowed={true} /> */}
            </Container>
        </Loadable>
    )
}
