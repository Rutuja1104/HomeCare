...
import { deleteTaskAndSchedule, getTaskListByEpisodeId } from '../../AdminEpisodeManagementSaga';
...
const TaskList = () => {
    ...
    const { TaskListPaginationState, selectedTasks, taskList, showDeleteTaskModal, selectedTask } = useSelector(
        ...
        if (episodeId) {
            dispatch(
                getTaskListByEpisodeId({
                    agencyId,
                    episodeId,
                    status: TaskListPaginationState.Status.value,
                    limit: TaskListPaginationState.PageSize,
                    pageNumber: TaskListPaginationState.PageNumber,
                    token
                })
            );
        }
    ...
    const ACTION_BUTTONS = [
        {
            ...
            onClickCb: (row) => {
                navigate(
                    `/${EPISODE_MANAGEMENT.EPISODE_MANAGEMENT}/${EPISODE_MANAGEMENT.CHILD_ROUTS.CREATE_TASK}/${episodeId}?ti=${row.id}&ie=true`
                );
            }
        },
        {
            text: 'Delete Task',
            disabled: (row) => row.status === 'Open' || 'InProgress',
            // disabled:  ['Open', 'InProgress'].includes(TaskListPaginationState.Status.value),
            onClickCb: (row) => {
    ...
    return (
        <React.Fragment>
            <div className="mb-2 d-flex justify-content-between">
                ...
                />
                <Button
                    styles={{ whiteSpace: 'nowrap', marginLeft: '20px' }}
                    variant={BUTTON_TYPE.PRIMARY}
                    onClickCb={() =>
                        navigate(
                            `/${EPISODE_MANAGEMENT.EPISODE_MANAGEMENT}/${EPISODE_MANAGEMENT.CHILD_ROUTS.CREATE_TASK}/${episodeId}`
                        )
                    }
                >
                    Add Task
            ...
            <RichGrid
                ...
                paginationProps={{
                    ...
                    total: TaskListPaginationState.totalTasks,
                    onChangeTakeCb: (newTake) => {
                        dispatch(
                            setTaskListPaginationState({ ...TaskListPaginationState, PageNumber: 1, PageSize: newTake })
                        );
                        dispatch(
                            getTaskListByEpisodeId({
                                agencyId,
