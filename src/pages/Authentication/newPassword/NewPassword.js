...
import { postSetNewPassword } from '../AuthenticationSaga';
...
const NewPassword = () => {
    ...
    const handleSetNewPasswordClick = () => {
        ...
        if (!GeneralValidator.validateRequiredFields(authenticationDetails.newPassword)) {
            dispatch(postSetNewPassword({ userId: paramValue, data: { password: authenticationDetails?.newPassword?.password?.value, confirmPassword: authenticationDetails?.newPassword?.confirmPassword?.value } }))
        }
    }
        ...
        <React.Fragment>
            <div className='login-container'>
                ...
                <div className='login-details'>
                    <Heading type={HEADING.H1}>New Password</Heading>
                    ...
                    <Button variant={BUTTON_TYPE.PRIMARY_LIGHT_WITH_BORDER} className="login-button" onClickCb={handleSetNewPasswordClick} >Submit</Button>
                </div>
