...
import { componentKey as DataLoader } from "../loaders/dataloader/DataLoaderSlice";
import { getAgencyDetailsById } from "../loaders/dataloader/DataLoaderSaga";

function ConflictOfInterest() {
    const dispatch = useDispatch();
    const { applicationId, agencyId } = useParams();
    const { agencyDetail } = useSelector((state) => state[DataLoader]);
    ...
    useEffect(() => {
        ...
        dispatch(getNurseOnboardingFormDetails({ agencyId, applicationId, formType: "conflictOfInterest" }));
    }, []);
    ...
    const onChangeHandler = (event, rules) => {
        ...
        if (rules) {
            ...
            dispatch(setConflictOfInterestForm({ [name]: { value, errors, rules } }));
        } else {
            dispatch(setConflictOfInterestForm({ [name]: { value } }));
        }
    };

    const handleSaveSignatureClick = (sign, isCanvasEmpty) => {
        dispatch(setConflictOfInterestRequiredFieldsTouched(true));

        if (!isCanvasEmpty) {
            dispatch(setConflictOfInterest(sign));
            dispatch(
                postAppendDocument({
    ...
    return (
        <div className="nurse-onboarding-new-forms">
            <div className="conflictOfInterest" style={{ textAlign: "center", fontSize: "25px" }}>
            ...
            <ol className="paragraph-without-flex ">
                ...
                <li className="margin-bottom">
                    In matters involving a conflict of interest, a Board member or staff member must disclose any known
                    significant reasons why a transaction may not be in the best interest of the agency
                </li>
                <li className="margin-bottom">
            ...
            <div className="paragraph-without-flex conflictOfInterestContent" style={{ fontWeight: 600 }}>
            ...
            <div className="conflictOfInterestContent" style={{ display: "flex", alignItems: "center" }}>
                <span>
                    <CheckboxWLabel
                        ...
                        name="isRememberMe"
                        onChangeCb={() => {
                            dispatch(setConflictOfInterestFirstCheckbox(!ConflictOfInterest.checkBoxFirst));
                        }}
                        value={ConflictOfInterest.checkBoxFirst}
                    />
                </span>
                I herby affirm that I know of no issues that would present a conflict of interest arising from any
                situation related to my involvement/association with {agencyDetail?.name}
            </div>
            <br />
            <div className="conflictOfInterestContent" style={{ display: "flex", alignItems: "center" }}>
                <span>
                    <CheckboxWLabel
                        ...
                        name="isRememberMe"
                        onChangeCb={() => {
                            dispatch(setConflictOfInterestSecondCheckbox(!ConflictOfInterest.checkBoxSecond));
                        }}
            ...
            <ResponsiveBox size={300}>
                ...
                />
                {/* <SignaturePad saveSignatureCb={handleSaveSignatureClick} savedSignature={ConflictOfInterest.sign}/> */}
                <div></div>
                <div>
                    <DatePicker
                        label="Date :"
